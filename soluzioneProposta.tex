...
\begin{algorithm}[h!]
	...
	\lnl{ALG-Tr1} Learn filters $\{\mb d_m\}$, compute $\overline{\mb g}, \Sigma$ from $\mb g(i)$ extracted from $S$. \\
	\lnl{ALG-Tr2} Set a threshold $\gamma > 0$.\\
	\textbf{Anomaly Detection in a test image $\mb s$}:\\
	\lnl{ALG-Test1} Compute the filter maps $\{\mb x_m\}$ solving~\eqref{eq:conv_sparse_coding}. \\
	\lnl{ALG-Test2} \ForEach{\textnormal{pixel} $i$ \textnormal{of} $\mb s$}
	{
		\lnl{ALG-Test3} Compute $\mb g(i)$\\
		\lnl{ALG-Test4} \If{$\big(\mb g(i) - \overline{\mb g})' \Sigma^{-1}(\mb g(i) - \overline{\mb g}\big) > \gamma$}
		{
			\lnl{ALG-Test5} $i$ belongs to an anomalous region\\
		}
		%{
		%\lnl{ALG-Test6} $i$ belongs to a normal region\\
		%}
	}   
	%\caption{Anomaly detection using convolutional sparsity.}
	\label{alg:ALG1}
\end{algorithm}
