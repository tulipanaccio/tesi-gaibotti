...
\begin{array}{lcl}
\Gamma_{min}^g & = & \hat{\mu}_g -\gamma \hat{\sigma}_g\\
\Gamma_{max}^g & = & \hat{\mu}_g + \gamma \hat{\sigma}_g
\end{array},
\end{equation}
dove $\hat{\mu}_g$ indica il valore medio delle osservazioni del training set
\begin{equation}
	\hat{\mu}_g = \frac{\sum_{\tau = 1}^{T_{o}} \frac{\partial g}{\partial t}(\tau)}{T_{o}}, \nonumber
\end{equation}
$\hat{\sigma}_g$ indica la deviazione standard delle osservazioni del training set
\begin{equation}
\hat{\sigma}_g  = \sqrt{\frac{1}{T_{o}-1}\sum_{\tau=1}^{T_{o}}\left(\frac{\partial g}{\partial t}(\tau) - \hat{\mu}_g(\tau)\right)^2} \nonumber
\end{equation}
